...
export async function createEditor(container: HTMLElement) {
  ...
  const b = new NumberNode(100, process);
  ...
  await editor.addConnection(new Connection(a, 'value', brush, 'a'));
  await editor.addConnection(new Connection(b, 'value', brush, 'b'));
...
// Will need an upgrade soon!
  async function process() {
    ...
    editor
      .getNodes()
      .filter((node) => node instanceof BrushNode)
      .forEach(async (node) => {
